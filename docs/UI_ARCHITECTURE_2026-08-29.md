...
| 02 | 문서별 4단계 파이프라인 보드 | 적용 |
| 03 | 디렉토리 루트 순차 공개 | 적용 |
| 04 | 페이지 단위 OCR 스트림 + 라이브 판독 화면 | 적용 (코드·테스트), **실기기 미검증** |
...
```
nextjs:      tsc · eslint 0 · 37 files / 204 tests · next build · playwright 50 (7폭 + reduced-motion)
ocr worker:  python -m pytest  → 12
cdr worker:  npm test          → 42
```
