...
| SD-14 | **Research receipts R-01 and R-02 are published byte-identical** under `/research/receipts/` with their full sha256, including the internal benchmark dataset path names R-02 contains. Scanned: no credentials, no drive paths, no addresses. Rewriting a receipt to hide a path would change its hash and break the claim it backs. | Historical evidence is never rewritten (constitution). |
