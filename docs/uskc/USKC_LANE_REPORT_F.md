...
- **`connector-oauth-adapters.ts` — the Drive `trashed = false` query is at line 80.** The gap is real and verified: Drive drops trashed files silently while Dropbox (`:114`) and Graph (`:154`) emit `kind: "deleted"`. (Re-checked at integration: `grep -n trashed nextjs/lib/connector-oauth-adapters.ts` → `80:  url.searchParams.set("q", "trashed = false");`.)
