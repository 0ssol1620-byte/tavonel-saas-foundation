...
values ('92929292-9292-4292-8292-929292929292','pilot-cptest','dropbox','Checkpoint test','worker-sync-v2-aaaaaaaaaaaaaaaa',
...
begin
  ...
  perform public.complete_foundation_job_batch('pilot-cptest',j,'worker-sync-v2-aaaaaaaaaaaaaaaa','succeeded',1,1,'cursor-one');
  ...
  begin
    perform public.complete_foundation_job_batch('pilot-cptest','job-'||repeat('d',32),'worker-sync-v2-aaaaaaaaaaaaaaaa','succeeded',1,1,'https://invalid.example');
  ...
  update public.foundation_jobs set lease_expires_at=now()-interval '1 second'
    where workspace_key='pilot-cptest' and job_id='job-'||repeat('d',32);
  begin
    perform public.complete_foundation_job_batch('pilot-cptest','job-'||repeat('d',32),'worker-sync-v2-aaaaaaaaaaaaaaaa','succeeded',1,1,'cursor-two');
  ...
  assert (select cursor_token='cursor-one' from public.foundation_connector_checkpoints where workspace_key='pilot-cptest'), 'expired lease preserves watermark';
  perform public.complete_foundation_job_batch('pilot-cptest','job-'||repeat('d',32),'worker-sync-v2-aaaaaaaaaaaaaaaa','failed',0,0,null,120,'TEST_FAILED');
