-- Each Vercel cron invocation executes exactly one bounded batch. A progress report must
-- release its lease immediately so the next invocation can claim the next provider page.
begin;
