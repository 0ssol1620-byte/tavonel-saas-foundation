-- Quotas are scheduling constraints, not execution failures. A source import that reaches
-- its rolling daily admission limit must keep its cursor and retry budget while it waits for
-- capacity to reopen; treating that wait as a retry eventually parks a healthy corpus dead.
begin;
...
begin
  if p_outcome not in ('progress', 'succeeded', 'deferred', 'retry', 'failed') then
  ...
  if p_outcome = 'deferred' then
    ...
    v_defer_seconds := least(greatest(coalesce(p_lease_seconds, 3600), 60), 86400);
    update public.foundation_jobs
    set state = 'queued',
        attempt = 0,
        leased_by = null,
        lease_expires_at = null,
        available_at = now() + make_interval(secs => v_defer_seconds),
        error_code = p_error_code,
        error_detail = left(p_error_detail, 500),
        completed_at = null,
        updated_at = now()
    where workspace_key = p_workspace_key and job_id = p_job_id;
    return jsonb_build_object('state', 'queued', 'retry_in_seconds', v_defer_seconds);
  end if;
