-- Complete R2-prefix inventory attestation before physical source deletion.
-- The producer lists and hashes every object under the exact document prefixes. The database
-- independently validates source/document ownership, quiescence, key scope and the manifest digest.
begin;
  ...
  if exists (
    ...
    select 1 from public.foundation_compile_jobs j
     where j.workspace_key = v_tombstone.workspace_key
       and j.document_ids && v_document_ids
...
begin
  ...
  if v_count <> v_size then raise exception 'SOURCE_DELETION_INVENTORY_DUPLICATE_KEY'; end if;
