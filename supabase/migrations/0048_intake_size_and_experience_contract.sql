-- Real-document intake contract.
-- Browser-direct R2 transfer means the application server never carries source bytes, so the
-- historical 5 MiB ceiling was not a safety boundary; it only rejected ordinary manuals.
-- Keep free evaluation tightly bounded while allowing paid/owner workspaces to ingest practical
-- technical-document sizes. Page/compute reservations remain independent and still gate OCR cost.
begin;
...
begin
  ...
  if p_requested_bytes > 262144000 then -- 250 MiB
  ...
  if not v_owner then
    ...
    if found then
      ...
      if p_requested_bytes > 52428800 then -- 50 MiB
      ...
      v_day_byte_limit := 157286400; -- 150 MiB, max three 50 MiB files
    end if;
  end if;
