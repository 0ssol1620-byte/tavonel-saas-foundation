-- Permit a byte-different Paddle replay only for a previously persisted,
-- semantically identical stale Studio-upgrade event. Processed events retain
-- strict payload-digest conflict detection.
begin;
...
begin
  ...
  if found then
    ...
    if existing.processing_result <> 'stale_or_mismatched_subscription' then
    ...
    if p_action <> 'subscription' then
      return jsonb_build_object('status', existing.processing_result, 'eventId', p_event_id);
    end if;
