...
create type public.foundation_intake_state as enum (
  'requested',
  'quarantined',
  'sanitized',
  'ocr_ready',
...
create table public.foundation_intake_admissions (
  workspace_key text not null check (workspace_key ~ '^pilot-[A-Za-z0-9]{1,32}$'),
  ...
  state public.foundation_intake_state not null default 'requested',
  expires_at timestamptz not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  primary key (workspace_key, document_id),
  check (object_key = 'quarantine/' || workspace_key || '/' || document_id::text || '/source'),
  check (expires_at > created_at and expires_at <= created_at + interval '15 minutes')
);

create index foundation_intake_admissions_workspace_created_idx
  on public.foundation_intake_admissions (workspace_key, created_at desc);

alter table public.foundation_intake_admissions enable row level security;
revoke all on public.foundation_intake_admissions from anon, authenticated;
...
begin
  ...
  if found then
    if existing.user_id <> p_user_id
      ...
      or existing.requested_bytes <> p_requested_bytes
  ...
  if day_count >= 20 or day_bytes + p_requested_bytes > 104857600 then
