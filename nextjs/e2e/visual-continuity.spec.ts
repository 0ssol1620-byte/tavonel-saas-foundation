...
async function runCut4Test(
  ...
  const page = await context.newPage();
    ...
    if (reducedMotion) {
      ...
      const canvasCount = await page.locator(".compile-film-live canvas.film-canvas").count();
      ...
      const backing = await page.evaluate(() => {
        const c = document.querySelector(".compile-film-live canvas.film-canvas") as HTMLCanvasElement;
