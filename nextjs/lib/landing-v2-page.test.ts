...
import { BARRED, LANDING_V2_FORBIDDEN, OVERCLAIMS, RETIRED_NAMES } from "./landing-v2-copy.test";
...
/*
  ...
  they are assembled into: that the nine landmarks are present once each, in §9's order, with the
...
describe("landing v2 -- the composition", () => {
  it.each(LOCALES)("%s renders §9's nine scenes, once each, in order", (locale) => {
    ...
    expect(sections.map((open) => attr(open, "id"))).toEqual([...LANDING_V2_SCENE_ORDER]);
    expect(sections.map((open) => attr(open, "data-scene"))).toEqual(
      LANDING_V2_SCENE_ORDER.map((_, index) => String(index + 1)),
    );
  ...
  it.each(LOCALES)("%s has one h1, and it is the founder-owned headline", (locale) => {
    ...
    */
    ...
    const expected = locale === "ko" ? LANDING_V2_COPY.ko.hero.headline : BRAND_LINE.headline;
    expect(bare(heading.replace(/<[^>]*>/g, ""))).toBe(bare(expected));
  });

  it.each(LOCALES)("%s gives each scene below the hero its own h2, and skips no level", (locale) => {
    const main = mainOf(render(locale));
    const levels = [...main.matchAll(/<h([1-6])(\s[^>]*)?>/g)].map((match) => Number(match[1]));
    expect(levels.filter((level) => level === 2)).toHaveLength(8);
  ...
  it.each(LOCALES)("%s claims no id twice, and every aria-labelledby resolves", (locale) => {
    const main = mainOf(render(locale));
  ...
  */
  it.each(LOCALES)("%s makes no claim §20.2, BARRED or OVERCLAIMS forbids", (locale) => {
    const text = mainOf(render(locale)).replace(/<[^>]*>/g, " ").toLowerCase();
    for (const phrase of [...LANDING_V2_FORBIDDEN, ...BARRED, ...OVERCLAIMS]) {
      expect(text, `the rendered page says "${phrase}"`).not.toContain(phrase.toLowerCase());
    }
  });
  ...
  it.each(LOCALES)("%s uses no retired name", (locale) => {
    ...
    for (const name of RETIRED_NAMES) {
      expect(text, `the rendered page uses the retired name "${name}"`).not.toContain(name);
    }
  });
  ...
  */
  ...
  it.each(LOCALES)("%s plays no video and preloads exactly one image", (locale) => {
    const main = mainOf(render(locale));
    expect(main).not.toContain("<video");
    expect(main).not.toContain("<iframe");
    // §27: the LCP resource is one image, and only one element may claim the priority.
    const images = elements(main, "img");
    expect(images.filter((open) => attr(open, "fetchpriority") === "high")).toHaveLength(1);
  });
  ...
  */
  it.each(LOCALES)("%s declares every image's box, alt and loading posture", (locale) => {
    const main = mainOf(render(locale));
  ...
  /*
    ...
    spelling of it.
  */
  it.each(LOCALES)("%s prints the Entity caveat where a reader can see it", (locale) => {
