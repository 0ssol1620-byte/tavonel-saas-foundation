...
export const API_ERROR_GROUPS: readonly ApiErrorGroup[] = [
    ...
    codes: [
      {
        code: "AUTH_REQUIRED",
        status: 401,
        meaning: "No credential was presented, or the bearer token is not a key this deployment issued. This is what an unauthenticated request to any scoped route returns.",
      ...
      },
      {
        ...
        whatToDo: "Check you are sending the whole key, including the `tvnl_live_` prefix, and that it belongs to this deployment.",
      },
      {
      ...
      },
      {
        ...
        status: 403,
        meaning: "The credential is valid and the workspace it names is not admitted to this deployment.",
      ...
      },
      {
        ...
        whatToDo: "See /pricing. The bar is the same for all three on purpose: a plan that may promote and may not rebuild would leave its own Worlds answering from the fallback.",
      },
      {
        code: "SELF_SERVICE_NOT_ENABLED",
        meaning: "Self-serve purchase is not open on this deployment.",
  ...
  },
  {
    ...
    codes: [
      ...
      { code: "WORLD_PROMOTION_INVALID", status: 400, meaning: "The promotion request body did not validate.", whatToDo: "Promotion is a signed-in browser action; no API key holds it." },
      ...
      { code: "PROMOTION_METADATA_TOO_LARGE", status: 413, meaning: "The promotion note exceeded its bound.", whatToDo: "Shorten the note." },
      ...
      { code: "AUDIT_EXPORT_WINDOW_INVALID", status: 400, meaning: "The audit export window was not a range this deployment serves.", whatToDo: "Narrow the window." },
    ],
  },
  {
    ...
    codes: [
      ...
      { code: "INTAKE_DISABLED", status: 503, meaning: "Intake is closed on this deployment.", whatToDo: "Nothing a caller can send opens it. /api/status publishes the current state." },
      ...
      { code: "SIGNER_NOT_CONFIGURED", status: 503, meaning: "The upload URL signer is not configured on this deployment, so no capability can be issued.", whatToDo: "Nothing a caller can send. /api/status reports the deployment's state." },
      ...
      { code: "TRIAL_DISABLED", meaning: "Free evaluation is closed on this deployment.", whatToDo: "Nothing a caller can send." },
  ...
  },
  {
    ...
    codes: [
      { code: "ACTIVE_WORLD_NOT_FOUND", status: 409, meaning: "Nothing has been promoted for this collection, so there is no World to read, index or answer from. A candidate nobody accepted is not a smaller answer — it is a different one.", whatToDo: "Promote a candidate in a signed-in session. No key can promote." },
      { code: "ACTIVE_WORLD_CONFLICT", status: 409, meaning: "Two promotions raced; neither was applied silently.", whatToDo: "Re-read the World and retry the promotion." },
      ...
      { code: "ACTIVE_WORLD_ARTIFACT_INVALID", status: 422, meaning: "The promoted artifact failed validation on read, so it was refused rather than served partially.", whatToDo: "Recompile. Report it with the manifest digest." },
      ...
      { code: "WORLD_CANDIDATE_NOT_PROMOTABLE", status: 422, meaning: "The candidate failed the checks promotion requires.", whatToDo: "Read the validation report in the candidate; it names what failed." },
      ...
      { code: "WORLD_EQUIVALENCE_REFUSED", status: 409, meaning: "The promotion would have replaced the active World with one the equivalence check does not accept as the same subject.", whatToDo: "Review the diff before promoting." },
      ...
      { code: "PROMOTION_ROLE_REQUIRED", status: 403, meaning: "Promotion needs the workspace owner or admin role.", whatToDo: "Ask an owner or admin to promote." },
  ...
  },
  {
    ...
    codes: [
      ...
      { code: "R2_NOT_CONFIGURED", status: 503, meaning: "Object storage is not configured on this deployment.", whatToDo: "Nothing a caller can send." },
  ...
  },
  {
    ...
    codes: [
      ...
      { code: "OAUTH_PROVIDER_NOT_CONFIGURED", status: 503, meaning: "The provider's client is not configured on this deployment. Fails closed rather than starting an authorization that cannot complete.", whatToDo: "Check /integrations for which providers are live here." },
      ...
      { code: "OAUTH_PROVIDER_INVALID", meaning: "The callback path named a provider this deployment does not have a connector for. Carried back on the workspace redirect rather than as a JSON body.", whatToDo: "Start the authorization from `POST /oauth-connectors/authorize`; do not construct the callback URL yourself." },
      { code: "OAUTH_CALLBACK_INVALID", status: 400, meaning: "The callback did not carry a state this deployment issued.", whatToDo: "Start the authorization again; do not construct the callback yourself." },
      ...
      { code: "JOB_LEASE_LOST", status: 409, meaning: "The worker's lease expired mid-run, so the turn was abandoned rather than finished under a lease it no longer held.", whatToDo: "Retry. Work is idempotent per batch." },
  ...
  },
  {
    ...
    codes: [
      ...
      { code: "DEVELOPER_STORE_NOT_CONFIGURED", status: 503, meaning: "The developer store is not configured on this deployment.", whatToDo: "Nothing a caller can send." },
