...
export const CONTRACT_CLAUSES: readonly ContractClause[] = [
  ...
  },
  {
    ...
    body:
      "FP-200, Feedwater Pump 200, Pump FP200 and P-200 are one piece of equipment in the plant and four strings in the corpus. An object here carries a stable key derived from content, so recompiling the same source lands on the same object rather than a new one. Merging two different strings into one object is the part TAVONEL does not do automatically: where the evidence does not settle identity, the compiler is required to leave it unresolved for a person rather than guess, and automatic resolution across sources is a direction.",
    state: "direction",
  ...
  },
  {
    ...
    body:
      "Every edge is typed and carries the evidence ids that justify it, and which types you get depends on which engine compiled: the fallback compiler emits three — a document discusses_topic, a document mentions_entity, and a claim is supported_by the evidence for one exact document version — while Core V2, the engine TAVONEL runs, has no topic object at all and emits three of its own: a claim supported_by its evidence, a claim mentions an entity, and one claim contradicts another. They are written as graph/relationships.csv under the header id, subject_id, predicate, object_id, evidence_ids, so a relation naming no evidence is not a row either emitter can produce, and the predicate list an artifact advertises is computed from the edges that artifact contains rather than copied from the blueprint. Where a type is a heuristic rather than read semantics, that is said and not softened: the fallback's topic edge comes from a small set of keyword rules and its entity edge from a capitalised-token scan, which makes those two document-level co-occurrence, and Core V2's entity mention is a regular-expression scan over the same text. A contradicts edge is a candidate, not a verdict — the compiler flags a pair whose numbers or whose polarity disagree inside one topic and one time reference, sends the candidate to review rather than resolving it, and knows nothing about jurisdiction, units or exception clauses; two claims phrased with different dates are never compared. A retrieval unit names the claims and entities found inside its own region, and the gate in front of retrieval rejects a unit with nothing bound to it; the unit carries no relation id. Projecting a relation into the retrieval unit an answer is built from is the rung TAVONEL does not build.",
    state: "demonstrated",
