...
export default function ReproducibilityPage() {
  return <PublicProofRegistry footer={<TrustNext from="/reproducibility" />} eyebrow="PUBLIC PROOF PROTOCOL" title="Rebuild the evidence, not the claim." state="FIXTURE VERIFIED · EXTERNAL BENCHMARK OPEN" summary="This page answers one question: can you rerun the same input and get the same bytes. A digest proves bytes; it does not prove semantic quality — what was measured is on /research/notes, the protocol a published number must satisfy is on /benchmarks, how a citation binds to its source is on /evidence, and the security and compliance status is on /trust. Customer results are published only with written consent, and none has been given, so no fixture here stands in for a customer outcome." sections={[
