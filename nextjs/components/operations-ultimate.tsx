  ...
  return (
    <div className={styles.root} id="workspace-runs">
      <section className={styles.preflight} aria-labelledby="preflight-title">
        ...
        <dl className={styles.metrics}><div><dt>Sources</dt><dd>{snapshot.sourceCount}</dd></div><div><dt>Read and ready</dt><dd>{snapshot.readyCount}</dd></div><div><dt>Running</dt><dd>{snapshot.runningCount}</dd></div><div><dt>Review</dt><dd>{snapshot.heldCount}</dd></div><div><dt>Cost</dt><dd title="No compute quote has been issued">—</dd></div></dl>
      ...
      <section className={styles.theater} aria-labelledby="run-theater-title">
        <div className={styles.head}><div><p>Run detail · no estimated progress</p><h2 id="run-theater-title">Every transition requires an object or receipt.</h2></div><div><span className={styles.nextAction}>{snapshot.nextAction}</span><span className={styles.streamState} role="status">{streamState}</span></div></div>
