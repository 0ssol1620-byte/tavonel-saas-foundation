...
- **Access control is a workspace membership checked server-side on every request.** There are no
  roles, no SSO and no seat model in the current service, so the account that owns a workspace is the
...
on a RunPod endpoint for which the current service pins no region, and `/subprocessors` says so rather
...
no independent penetration-test report for the current service.** An external penetration test is
...
2026-09-16 and are stated in the body above. They are Draft v2 wording and have not been reviewed
